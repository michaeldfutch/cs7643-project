...
class FoolingImage:
    def make_fooling_image(self, X, target_y, model, min_iter=0):
        """
        ...
        for it in range(max_iter):
            ...
            f = model.forward(X_fooling_var)

            if torch.argmax(f) == target_y and it > min_iter:
                print("iteration fooled:", it)
                break

            f = f[0, target_y]
            
            out = f.backward()
            grad_norm = torch.linalg.norm(X_fooling_var.grad)

            dx = learning_rate * X_fooling_var.grad / grad_norm
